...
#CN="SVS TLS Certificate,O=INTEL,L=SC,ST=SF,C=US"
...
#svs config aasAdmin.bearer.token $Bearer_token >/dev/null
...
create_roles() {
		...
		#local svs_rm_rid=$( create_user_roles "SVS" "RoleManager" ) #Get roleid
		#local svs_um_rid=$( create_user_roles "SVS" "UserManager" ) #Get roleid
		#local svs_urm_rid=$( create_user_roles "SVS" "UserRoleManager" ) #Get roleid
		#local svs_cu_rid=$( create_user_roles "SVS" "CacheUpdater" ) #Get roleid
		#local aas_role_id=$( create_user_roles "CMS" "CertApprover" "CN=SVS JWT Signing Certificate; SAN=127.0.0.1,localhost;CERTTYPE=JWT-Signing" )
		#ROLE_ID_TO_MAP=`echo "\"$cms_role_id""\",\"""$svs_cu_rid""\",\"""$aas_role_id\""`
...
if [ $status -ne 200 ]; then
	...
	echo $BEARER_TOKEN
	#svs config aas.bearer.token $BEARER_TOKEN >/dev/null
fi
